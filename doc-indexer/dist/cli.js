...
async function indexDocsDirectory(dirPath, extensions = [".md", ".txt"]) {
  const startTime = Date.now();
  const files = import_fs.readdirSync(dirPath);
  const allChunks = [];
  ...
  for (const file of files) {
    ...
    if (!extensions.includes(ext))
      continue;
    const fullPath = import_path.resolve(dirPath, file);
...
var STORAGE_PATH = import_path2.resolve(process.cwd(), ".unity-docs-index.json");
...
class DocSearch {
  ...
  async search(options) {
    ...
    const sorted = Array.from(combined.entries()).sort((a, b) => b[1] - a[1]).slice(0, topK).map(([id, score]) => {
