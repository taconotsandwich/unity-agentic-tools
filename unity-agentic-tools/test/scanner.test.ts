...
describeIfNative('UnityScanner', () => {
    ...
    describe('scan_scene_with_components', () => {
        it('should parse GameObjects with components', () => {
            ...
            expect(Array.isArray(gameobjects)).toBe(true);
            expect(gameobjects.length).toBeGreaterThan(0);
            // Each GameObject should have a name
            for (const go of gameobjects) {
                expect(typeof go.name).toBe('string');
                expect(go.name.length).toBeGreaterThan(0);
            }
        });
    });
    ...
    describe('inspect', () => {
        it.each([
            ['file ID', '162353359'],
            ['name', 'Instruction'],
        ])('should get GameObject by %s', (_label, identifier) => {
    ...
    describe('PrefabInstance awareness', () => {
        ...
        it.each([
            ['file ID', '700000'],
            ['name', 'MyEnemy'],
        ])('inspect PrefabInstance by %s should return correct type and name', (_label, identifier) => {
            const result = scanner.inspect({
    ...
    describe('read_asset', () => {
        ...
        it('should not crash on scene files and return an array', () => {
            // Scene files have GameObjects which are filtered out
            const objects = scanner.read_asset(MAIN_UNITY);
            expect(Array.isArray(objects)).toBe(true);
            // Scene files should not produce asset objects (class_id 1 = GameObject is filtered)
            for (const obj of objects) {
                expect(obj.class_id).not.toBe(1);
            }
        });
    });
