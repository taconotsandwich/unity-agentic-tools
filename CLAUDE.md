...
- **Focused discovery**: Prefer `list <query> --brief` before invoking commands. Inspect state with `query.object`, `query.scene`, `scene.hierarchy`, or `ui.snapshot` rather than broad command listings.
...
- **Discovery**: Unity writes `.unity-agentic/editor.json` (port + PID). The CLI falls back to `.unity-agentic/editor.last.json`, then scans ports 53782-53791 and matches the reported project path.
...
- **Event streaming**: Console, play/pause, and test notifications are broadcast to connected clients; `stream` filters topics and log types on the CLI side.
...
- **Ref system**: `RefManager.cs` maintains `@hN` (hierarchy) and `@uN` (UI) ref registries. Refs created by `scene.hierarchy`/`ui.snapshot` are cleared on scene change, play mode transition, or domain reload
...
- **Input System**: `unity-package/Editor/Util/Input.cs` uses `#if ENABLE_INPUT_SYSTEM` conditional compilation. Legacy Input returns an explicit error because its API is read-only.
- **Annotated screenshots**: `unity-package/Editor/Util/Screenshot.cs` captures via RenderTexture, composites numbered labels, and returns element refs.
