...
class UnityScanner {
    ...
    ensureGuidResolver(file) {
        if (!this.guidResolver) {
            const projectRoot = guid_resolver_1.GuidResolver.findProjectRoot((0, path_1.dirname)(file));
            if (projectRoot) {
                this.guidResolver = new guid_resolver_1.GuidResolver(projectRoot);
